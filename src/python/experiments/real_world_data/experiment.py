...
def _bag_graph_domain(
    ...
    return tuple(
        sorted(
            {
                graph_index
                for bag in bags.graph_indices
                for graph_index in bag
            }
        )
    )
...
def _perslay_sample_bounds(
    ...
    nonempty = tuple(
        ...
        for graph_index in graph_indices
        if dataset.persistence_diagrams[
            graph_index
        ].shape[
            0
        ]
...
def _perslay_bag_positions(
    bags: _BagPartition,
    graph_indices: Sequence[int],
) -> NDArray[np.int64]:
    positions = {
        graph_index: position
...
def _make_perslay_model(
    dataset: RealWorldDataset,
    training_graph_indices: Sequence[int],
    *,
    class_count: int,
    feature_count: int,
    seed: int,
...
def _fit_perslay(
    ...
    *,
    ...
    training_graph_indices = _bag_graph_domain(
        train_bags
    )
    ...
    bag_positions = torch.as_tensor(
        _perslay_bag_positions(
            train_bags,
            training_graph_indices,
        ),
        dtype=torch.long,
    ...
    encoded_labels = torch.as_tensor(
        np.asarray(
            tuple(
                ...
                for label in train_bags.labels
            ),
            dtype=np.int64,
        ),
        dtype=torch.long,
    ...
    optimizer = torch.optim.Adam(
        model.parameters(),
        lr=config.perslay.learning_rate,
    )

    loss_function = nn.CrossEntropyLoss()

    for _ in range(
        config.perslay.epochs
    ):
        model.train()
...
def _perslay_bag_features(
    ...
    *,
    ...
    graph_indices = _bag_graph_domain(
        bags
    )
...
def _run_perslay_bag_size(
    ...
    *,
    ...
    for outer_position, outer_fold in enumerate(
        ...
        _report(
            f"[Real world] {dataset.metadata.name}, "
            f"{PERSLAY_METHOD}, bag size {bag_size}: "
            f"outer {outer_position}/{len(plan.folds)}"
        )
        ...
        train_features = _perslay_bag_features(
            model,
            dataset,
            train_bags,
            feature_count=(
        ...
        test_features = _perslay_bag_features(
            model,
            dataset,
            test_bags,
            feature_count=(
